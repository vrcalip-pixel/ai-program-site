...
Five gestures: point (the index fingertip drives the cursor), hold (rest on a node or button for just over half a second; a ring fills, then it activates), an open hand swept to your left (next course; judged on the palm centre from the moment the hand is in view, and a frame or two of not-quite-open is forgiven), a fist kept in place for 0.7 s (back; a swipe right collided with the hand returning after a swipe left) and a throw aside (reset: open hand closes to a fist, moves to your left while closed, opens again within 0.6 s; the map returns to its starting view, the same as the Recenter button). Back and throw are exclusive: a fist that leaves a small circle around where it closed cannot become a back until the hand reopens, and a fist held past the throw window cannot become a throw. The interpretation is in `src/lib/gestures.ts` with unit tests in `tests/`; run them with `npm test`.
