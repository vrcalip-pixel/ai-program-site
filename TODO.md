...
11. **Hero videos (six)** — AI 40 and AI 45 done 2026-09-07 (Vincent's clips, each encoded to a 4.4 s crossfaded loop, MP4 + WebM + poster, ~0.8 MB each). Four to go: AI 60, 65, 70, 80. Hand a session the source MP4 and the course number; it runs `node scripts/hero-video.mjs <clip> ai-nn` and sets `video: true` in that course's frontmatter. Prompts are in BRIEF §9. Until then those heroes show the animated gradient with a "Hero video coming soon" tag.
...
17. **Tune the gestures from real use** — simplified 2026-09-07 to point, hold, swipe left (next) and fist (back) after the first set proved hard to perform. Open the map with `?hf=debug`, try each, and adjust `TUNE` in `src/components/HandsFree.astro` (or `localStorage.hf.tune` first, no rebuild). Likely knobs: `dwellMs` (hold time), `reach` (how much arm travel), `filterMinCutoff` (steadiness vs lag), `swipeMinDx` (how big a swipe), `fistHoldMs` (how long a fist is held before it counts).
