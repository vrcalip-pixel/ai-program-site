...
17. **Tune the gestures from real use** — simplified 2026-09-07 to point, hold, open-hand swipe left (next), fist in place (back) and, since 2026-09-08, palm-out pull-back (reset) after the first set proved hard to perform. Open the map with `?hf=debug`, try each, and adjust `TUNE` in `src/components/HandsFree.astro` (or `localStorage.hf.tune` first, no rebuild). Likely knobs: `dwellMs` (hold time), `reach` (how much arm travel), `filterMinCutoff` (steadiness vs lag), `swipeMinDx` and `swipeMinSpeed` (how big and how quick an open-hand sweep), `fistHoldMs` (how long a fist is kept in place before it counts; keep it above `throwWindow`), `fistStillRadius` and `fistSizeTolerance` (how far it may drift, or grow or shrink, and still count as in place), `pullShrink` (how much smaller the fist must be than the open palm for a reset), `pullMinSize` (how close the open palm must be to start one), `openGrace` (how long the swipe forgives a hand that reads as not-quite-open).
