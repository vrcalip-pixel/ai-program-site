...
17. **Tune the gestures from real use** — simplified 2026-09-07 to point, hold, swipe left, swipe right after the first set proved hard to perform. Open the map with `?hf=debug`, try each, and adjust `TUNE` in `src/components/HandsFree.astro` (or `localStorage.hf.tune` first, no rebuild). Likely knobs: `dwellMs` (hold time), `reach` (how much arm travel), `filterMinCutoff` (steadiness vs lag), `swipeMinDx` (how big a swipe).
